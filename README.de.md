...
> [!WARNING]
> Diese Extension ist Work in Progress und noch nicht produktionsreif. Nutzung auf eigene Gefahr.
...
### 4. Site Configuration erweitern (Alternative zu Environment-Variablen)

Falls du keine Environment-Variablen für die Zone ID verwendest, bearbeite `config/sites/{site-identifier}/config.yaml`:

```yaml
# Am Ende der Datei hinzufügen:
cloudflare:
  zoneId: 'deine-zone-id-hier'  # Optional wenn IGNITERCF_ZONE_* gesetzt ist
  enabled: true
```

**Beispiel für Multi-Domain (config.yaml):**
...
> **Hinweis:** Environment-Variablen haben Vorrang vor config.yaml Einstellungen.
...
Teste deine Cloudflare-Konfiguration (Token-Gültigkeit + Zone-Zugriff):
...
# Spezifische Site testen:
vendor/bin/typo3 ignitercf:test:connection main
```

**Via Backend-Modul:**
1. Gehe zu **System → IgniterCF → Configuration**
2. Klicke **Test** Button neben einer konfigurierten Site
3. Ergebnisse zeigen Token- und Zone-Status
...
**Lookup-Reihenfolge (Zone ID & API Token):**
1. `IGNITERCF_ZONE_{SITE}` / `IGNITERCF_TOKEN_{SITE}` (site-spezifisch)
2. `IGNITERCF_ZONE_ID` / `IGNITERCF_API_TOKEN` (globaler Fallback)
...
- Verbindungstest (Token + Zone-Verifizierung)
- Backend-Modul mit Statistiken und Konfigurations-Status
...
Teste Cloudflare API-Token und Zone-Zugriff für deine Sites:

```bash
# Alle konfigurierten Sites testen:
vendor/bin/typo3 ignitercf:test:connection
...
Ausgabe zeigt:
- Token-Gültigkeit (aktiv/abgelaufen/ungültig)
- Zone-Zugriff (erreichbar/verboten/nicht gefunden)
- Antwortzeit
